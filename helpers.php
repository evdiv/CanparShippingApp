...
function setShipmentVoidedInDB($ShipmentData) {
	...
	$pin = isset($ShipmentData['pin']) ? $ShipmentData['pin'] : '';
...
function getFilePathOnServer($txt = '', $type = 'label'){
	if(empty($txt)) {
		return '';
	}

	$labelName = explode(' ', microtime());
	$labelPath = "./labels/" . $labelName[1] . ".png";

	if($type == 'manifest') {
		$labelPath = "./manifests/" . $labelName[1] . ".pdf";
	}
    file_put_contents($labelPath, base64_decode($txt));
